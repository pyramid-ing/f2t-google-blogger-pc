...
export class BlogPostJobService implements JobProcessor {
  ...
  async process(jobId: string): Promise<void> {
    ...
    try {
      ...
      const blogOutline = await this.topicService.generateBlogOutline(job.blogPostJob.title, job.blogPostJob.content)
      ...
      const result = await this.publishService.publishPost(job.blogPostJob.title, detailedContent.sections[0].html)
      ...
      await this.prisma.job.update({
        ...
        data: {
          ...
          resultMessage: '포스팅이 성공적으로 생성되었습니다.',
        },
      })
